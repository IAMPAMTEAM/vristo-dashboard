...
const instructions = `너는 Knowlege의 excel 파일을 읽어 대화 시 필요한 분석을 수행하고 차트를 생성해줘야해.
...
const ChatContainer = () => {
    ...
    const {
        ...
        setFilteredItems,
        filteredItems,
    } = useChatContainer({ contactList, assistantId, threadId, openai, setRunId });
    ...
    useEffect(() => {
        const setupThread = async () => {
            const assistant = await openai.beta.assistants.create({
                ...
                tool_resources: {
                    code_interpreter: {
                        file_ids: ['file-RyYExNGdlLpfly8DkDvnebp9'],
                    },
                },
            });
    ...
    useEffect(() => {
        console.log(assistantId, threadId, runId);
        ...
        const checkStatusAndPrintMessages = async () => {
            ...
            if (runStatus.status === 'completed') {
                ...
                const message = messages.data[0].content[0] as { text: { value: string; annotations: any[] }; type: string };
                console.log(contactList[0].messages);
                contactList[0].messages.push({
                    fromUserId: 0,
                    toUserId: 1,
